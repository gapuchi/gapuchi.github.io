...
> This took me a second to understand what was happening here. `Person` doesn't have a `toJson` method, so the Scala compiler looks for a way for it to have it. By using the `implicit class`, the compiler passes in the instance of `Person` as part of the constructor and creates an instance of `JsonWriterOps`. With this implicit, `Person` can call `toJson`.

---

That's basically it! Using **type classes**, we can define functionality and it's implementations for specific types. It's a pretty cool pattern to implement. The only downside I see is it is not obvious what

```scala 
Person("Dave", "dave@example.com").toJson
```

does at initial glance. A `case class` doesn't have a `toJson` method, which might be confusing if a reader isn't aware of **type classes** or `implicit`'s.